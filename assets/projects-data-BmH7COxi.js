...
#include <MFRC522.h>
#define SS 5 #define RST 22
MFRC522 rfid(SS, RST); void setup() { SPI.begin(); rfid.PCD_Init(); Serial.begin(9600); } void loop() { if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) Serial.println("Asset Logged"); }`,149:"int air = 34; int noise = 35; void setup() { Serial.begin(9600); } void loop() { Serial.println(analogRead(air)); Serial.println(analogRead(noise)); delay(1500); }",150:"int traffic = 34; int l = 5; void setup() { pinMode(l, OUTPUT); } void loop() { digitalWrite(l, analogRead(traffic) > 2500 ? HIGH : LOW); }"},y={161:{pins:[{component:"Load Sensor",mappings:{arduino:"N/A",esp32:"GPIO 34"}},{component:"Non-Critical Relay",mappings:{arduino:"N/A",esp32:"GPIO 5"}}],code:i(`
            #include <WiFi.h>
        ...
        `)}};[T,f,y].forEach(t=>{Object.keys(t).forEach(e=>{a[e]||(a[e]={}),typeof t[e]=="string"?a[e].code=i(t[e]):a[e]={...a[e],...t[e]}})});const O=[...v,...I,...P].map(t=>a[t.id]?{...t,...a[t.id]}:t),W=O,R=[{title:"Voltage, Current & Resistance",content:"The holy trinity of electronics (Ohm's Law). Voltage is the pressure pushing electrons, Current is the flow of electrons, and Resistance is the opposition to that flow.",deepDive:"High current needs thick wires. High voltage needs insulation.",mistakes:"Shorting Power to Ground (Infinite Current = Fire).",tip:"Always check polarity before powering up."},{title:"Microcontrollers (The Brain)",content:"A small computer on a single chip. It reads inputs (sensors), processes data based on your code, and controls outputs (lights, motors). Common examples: Arduino Uno, ESP32, STM32.",deepDive:"They run firmware (C/C++), not a full OS like Windows.",mistakes:"Drawing too much current from a GPIO pin (>20mA).",tip:"Use transistors/MOSFETs to drive high-power loads."}];export{R as b,W as p};
