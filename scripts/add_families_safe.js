...
// Process each board - more careful approach
for (const [boardId, family] of Object.entries(families)) {
    // Match pattern: "board_id": {\n        "id": "board_id",\n        "name": "...",
    // Then add family after the name line
    ...
    if (match) {
        ...
        const afterMatch = content.substring(content.indexOf(match[0]) + match[0].length, content.indexOf(match[0]) + match[0].length + 200);
        if (!afterMatch.includes('"family":')) {
            content = content.replace(regex, `$1\n        "family": "${family}",`);
            console.log(`✓ ${boardId}`);
            successCount++;
        } else {
            console.log(`⊘ ${boardId} (already has family)`);
        }
    } else {
